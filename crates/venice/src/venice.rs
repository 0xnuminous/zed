...
pub const VENICE_API_URL: &str = "https://api.venice.ai/v1";
...
pub enum Model {
    #[serde(rename = "venice/auto")]
    #[default]
    Auto,
    #[serde(rename = "venice/tool")]
    Tool,
    #[serde(rename = "custom")]
    Custom {
        name: String,
...
impl Model {
    ...
    pub fn new(
        ...
        Self::Custom {
            ...
            max_tokens: max_tokens.unwrap_or(2_000_000),
            supports_tools,
            supports_images,
        }
    }
    ...
    pub fn id(&self) -> &str {
        match self {
            Self::Auto => "venice/auto",
            Self::Tool => "venice/tool",
...
pub async fn complete(
    ...
    if response.status().is_success() {
        ...
        match serde_json::from_str::<VeniceResponse>(&body) {
            Ok(response) if !response.error.message.is_empty() => {
                ...
                Err(anyhow!(
                    "Failed to connect to Venice API: {}",
                    error_message
                ))
            }
            _ => Err(anyhow!(
...
pub async fn list_models(client: &dyn HttpClient, api_url: &str) -> Result<Vec<Model>> {
    ...
    if response.status().is_success() {
        let response: ListModelsResponse =
            serde_json::from_str(&body).context("Unable to parse Venice models response")?;
        ...
        let models = response
            ...
            .map(|entry| {
                ...
                Model::new(
                    ...
                    Some(entry.supported_parameters.contains(&"tools".to_string())),
                    Some(
                        entry
                            .architecture
                            .as_ref()
